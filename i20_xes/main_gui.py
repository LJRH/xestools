...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # RXES controls replot on change
    ...
    def set_dataset(self, dataset: Optional[DataSet]):
        ...
        self.plot.ensure_line_count(1)  # start with one line; removable
    ...
    def on_save_spectrum(self):
        ...
        # Build combined array: [X1,Y1,X2,Y2,...], padded with NaN as needed
        maxlen = max(len(p[0]) for p in profiles)
        cols = []
        headers = []
        for (xv, yv, lbl) in profiles:
            pad = maxlen - len(xv)
            if pad > 0:
            ...
            cols.append(xv)
            cols.append(yv)
            headers.append(f"X {lbl}")
            headers.append(f"Y {lbl}")
        out = np.column_stack(cols)

        default_name = "profiles.csv"
        path, _ = QFileDialog.getSaveFileName(
            self, "Save spectra as CSV", default_name, "CSV (*.csv);;All files (*)"
        )
        if not path:
            return
        try:
    ...
    def update_profiles(self):
        ...
        if self.rxes_panel.rb_extr_incident.isChecked():
            ...
            if incident_view:
                # Incident Energy view (X=Ω, Y=ω): Constant Ω -> profile vs ω
                ...
                # Energy Transfer view (X=Ω, Y=Δ=Ω−ω): Constant Ω -> profile vs Δ
                # Build Δ axis from the Y2D grid (row-wise median Δ for robustness)
    ...
    def on_save_nexus(self):
        ...
        if not io_mod.H5_AVAILABLE:
            ...
            return
        start = os.path.join(os.path.dirname(self.dataset.source), "data.nxs") if self.dataset.source else "data.nxs"
        path, _ = QFileDialog.getSaveFileName(
