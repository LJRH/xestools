...
class BackgroundDialog(QtWidgets.QDialog):
    def __init__(
        ...
        self._cids: List[int] = []  # store mpl connection ids

        self._plot_data()        # sets initial plot and autoscale
        self._create_span()      # create span AFTER initial plot
        ...
        right.addWidget(QtWidgets.QLabel("Excluded regions"))
        right.addWidget(self.tbl, 1)
        rb = QtWidgets.QHBoxLayout(); rb.addWidget(btn_add); rb.addWidget(btn_clear)
        right.addLayout(rb)
        right.addWidget(btn_fit)
        row.addLayout(right, 1)

        top.addLayout(row)
        top.addWidget(self.report, 1)
        buttons = QtWidgets.QHBoxLayout(); buttons.addStretch(1); buttons.addWidget(btn_ok); buttons.addWidget(btn_cancel)
        top.addLayout(buttons)

        # Event bindings (store IDs so we can disconnect on close)
    ...
    def _create_span(self):
        # Disable blitting for stability; recreate each time axes are rebuilt
        try:
    ...
    def _plot_data(self):
        # Always destroy the old span before clearing axes
        self._destroy_span()
        ...
        # Recreate span on the fresh axes
        self._create_span()
    ...
    def _fit_background(self):
        ...
        self._plot_data()  # destroys/recreates span on fresh axes
    ...
    def closeEvent(self, event):
        # Clean up mpl bindings to prevent callbacks after the dialog is gone
        self._destroy_span()
